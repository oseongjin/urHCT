...
def find_faces_dnn(image):
    ...
    if save != []:
        ...
        #print("find face routin not run!")
        return before_positions,frame
...
def standing_check(x_que,y_que,face_pos,first_move_check):
    ...
    if len(x_que) == WAITTIME:
        tmp = False
        # Check last 50 list and 50 all same == True  else == False
...
def handle_client(client_socket, addr):
    ...
    if len(data) !=0:
        if data == 'Close':
            ...
            saveData = data.split()
            print(saveData)
            m_per_pixel = float(saveData[1])
        elif "hor" in data:
            saveData = data.split()
            print(saveData)
            hor_rot_max = math.radians(int(saveData[1]))
        elif "ver" in data:
            saveData = data.split()
            print(saveData)
            vert_rot_max = math.radians(int(saveData[1]))
        elif "distance" in data:
            saveData = data.split()
            print(saveData)
            DETECT_RANGE = int(saveData[1])
        elif "find" in data:
            saveData = data.split()
            print(saveData)
            FIND_PEOPLE = int(saveData[1])
        else:
            pass
    else:
        pass
...
# Socket Loop Start for Gui (hpnote)
th1 = Thread(target = socket_listen)
th1.start()

# Check complete ready for start robot
robot.reset_error()
