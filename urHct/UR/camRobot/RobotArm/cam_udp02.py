...
def show_frame(frame):
    ...
    try:
        dsp_sock.sendto( tmpstr.encode("utf-8"), (DSP_IP, int(DSP_PORT)))
        dsp_sock.sendto(stringData, (DSP_IP, int(DSP_PORT)))
        rblogger.info(type(tmpstr.encode("utf-8")))
        rblogger.info(type(stringData))
    except Exception as e:
        #dsp_sock.close()
        rblogger.info(str(e))
...
dsp_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
