    ...
    if save != []:
        ...
        try:
            ...
            for q in land_result.reshape(-1,70):
                ...
                x_nose = int(q[8] * land_frame.shape[1])
                y_nose = int(q[9] * land_frame.shape[0])
            #rblogger.debug("land_result {}".format(land_result))
            
            leftEye = (lx, ly)
            rightEye = (rx, ry)
            nose = (x_nose, y_nose)
            final_nose = (startX + nose[0], startY + nose[1])
            ...
            dist  = tddata[2]
            htemp = tddata[3]
            xpos = final_nose[0] - video_midpoint[0]
            ypos = final_nose[1] - video_midpoint[1]
            """
            if abs(xpos) < 20 and abs(ypos) < 20 and dis < 100:
                cap_frame = compress_frame(image)
                position_from_center = (xpos, ypos, dis, dist, htemp, cap_frame)
            else:
            ...
            cv2.putText(frame, temp_text, (50, 100),
                        cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 3)
            cv2.line(frame, video_midpoint, final_nose, (0, 200, 0), 5)
