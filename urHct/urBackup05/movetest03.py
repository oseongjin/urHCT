...
ctrlogger.debug("emissivity {}".format(emissivity))
...
def check_max_xy(xy_coord):
    x_y = [0,0,0]
    ctrlogger.debug("xy before conversion: {}".format(xy_coord))
    ...
    ctrlogger.debug("xy after conversion: ".format(x_y))
...
def standing_check(face_pos):
    ctrlogger.debug('standing in')
    global xCoord
    global yCoord
    xCoord.append(abs(face_pos[0]))
    yCoord.append(abs(face_pos[1]))
    # max waittime
    if len(xCoord) == WAITTIME:
        tmp = False
        ...
        if tmp == True:
            ...
            # detect center
            if sum(xCoord)/WAITTIME < 50 and sum(yCoord)/WAITTIME < 50:
                ctrlogger.debug("center")
            else :
                pass
    else:
        ctrlogger.debug('else')
        pass
...
def compare_joint():
    ...
    joint = joint.tolist()
    joint.sort()
    ctrlogger.debug("joint : {}, init_radians : {}".format(joint, init_radians))
...
try:
    ...
    server_socket.bind((CAM_HOST, int(CAM_PORT)))
    server_socket.listen(5)
    client_socket, addr = server_socket.accept()
    ctrlogger.debug("Connected by {}".format(addr))
    while True:
            ...
            if len(xCoord) > 5:
                ...
                if move == False:
                    ...
                    if once == True:
                        ...
                        if face_pose[0] < 50 and face_pose[1] < 50 and ddis < 150:
                            temperature = (ttemp * emissivity) / 100
