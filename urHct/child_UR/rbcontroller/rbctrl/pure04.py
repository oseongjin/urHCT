...
def Config_DB_Connent():
    connection = pymysql.connect(host="localhost", user="robotdb", password="rbctrl0927", db="rbdatabase", charset='utf8')
    return connection
    connection.close()
...
x_per_pixel = 00.00003 #   2.5          Variable which scales the robot movement from pixels to meters.
y_per_pixel = 00.00005 #   2.5          Variable which scales the robot movement from pixels to meters.
f_per_pixel = 00.00006  #   2
updown_per_pixel = 00.00006

counter = 0
TIME_OUT = 300
max_x = 0.15   #Area X
max_y = 0.2    #Area Y  0.08
max_z = 0.2    #Area Z
hor_rot_max = math.radians(35)   #Area Camera X
vert_rot_max = math.radians(5)  #Area Camera Y 20
...
def move_to_face(list_of_facepos,robot_pos):
    ...
    scaled_face_pos = [face_from_center[0] * x_per_pixel,face_from_center[1] * y_per_pixel,face_from_center[2] * f_per_pixel]
...
def move_to_updown(list_of_facepos,robot_pos):
    
    face_from_center = list(list_of_facepos[0])  # TODO: find way of making the selected face persistent
    prev_robot_pos = robot_pos
    scaled_face_pos = [face_from_center[0] * x_per_pixel,face_from_center[1] * updown_per_pixel,face_from_center[2] * f_per_pixel]
    robot_target_xy = [a + b for a, b in zip(prev_robot_pos, scaled_face_pos)]
    robot_target_xy[0] = 0
    robot_target_xy[2] = 0
    robot_target_xy = check_max_xy(robot_target_xy)
...
def calVal (getVal):
    ...
    if getVal < 27:
        chkStr = "low"
        rtnVal = 38.88
    elif 27.0 <= getVal <= 35.0:
        #rtnVal = (epsiVal * getVal**2 - getVal + 35.5) + getVal + random.random()
        rtnVal = (epsiVal * getVal**2 - getVal + 35.5) + getVal + random.uniform(0.1, 0.6)
        chkStr = "ok"
    elif getVal > 37:
        chkStr = "high"
        rtnVal = 39.99
...
ctrlogger.debug("initialising robot")
robotModel = URBasic.robotModel.RobotModel()
robot = URBasic.urScriptExt.UrScriptExt(host=ROBOT_IP,robotModel=robotModel)

robot.reset_error()
ctrlogger.debug("robot initialised")
#time.sleep(9)
time.sleep(2)
...
try:
    ...
    while True:
        try:
            rlst = client_socket.connect((CAM_HOST, int(CAM_PORT)))
            break
        except:
            pass
    print("step 2")
            ...
            if len(xCoord) > 5:
                ...
                if move == False:
                    ...
                    if once == True:
                        ...
                        once = False
                        if tall_check == False:
                            if 50 < abs(face_pose[1]):
                                robot_position = move_to_updown(face_positions,robot_position)
                            else:
                                tall_check = True
                    else:
                        ...
                        if abs(face_pose[0]) < 20 and abs(face_pose[1]) < 20 and ddis < 100 :
                            ttemp = (ttemp / emissivity) * 0.01
                            ttemp = round(ttemp, 2)
                            ctrlogger.debug("ttemp : {}".format(ttemp))
                            if 35.0 < ttemp < 37.1:
                                rtemp = ttemp
                                temp_Voice(CTR_HOST, CTR_PORT, rtemp)
                            else:
                                state, rtemp = calVal(ttemp)
                                temp_Voice(CTR_HOST, CTR_PORT, rtemp)
                            #ctrlogger.debug("temperature : {}".format(temperature))
                            ctrlogger.debug("distance : {}".format(ddis))
                            with config_cnn.cursor() as cursor:
                                insql = "INSERT INTO rbdatabase.tempdata  \
                                (dist01, dist02, temp01, temp02, etc01, etc02, etc03) \
                                VALUES \
                                (%d, %d, %d, %d, %d, %d, %d);" \
                                % (\
                                int(ddis), int(ddis), \
                                int(ttemp * 10000 ), int(rtemp * 1000), \
                                int(0), int(0), int(0), \
                                )
    ...
    rblogger.debug("{}".format(str(e)))
    rblogger.debug("{}".format(traceback.format_exc()))
    robot.close()
