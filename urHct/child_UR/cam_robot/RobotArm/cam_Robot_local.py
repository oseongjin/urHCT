...
def find_faces_dnn(image, tddata):
    ...
    try:
        frame = cv2.resize(image, (672, 504), cv2.INTER_AREA)
           
        blob = cv2.dnn.blobFromImage(frame, size=(300, 300), ddepth=cv2.CV_8U)
        pretrained_model.setInput(blob)
        detections = pretrained_model.forward()
    except Exception as e:
        rblogger.error("CV2 exception {}".format(e))
    ...
    if save != []:
        ...
        try:
            ...
            rblogger.error("Find Face DNN routin exception {}".format(e))
